...
class Monitor():
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction == 1:  
            ...
            self.car_waiting_north.wait_for(self.max_waiting_car_north)
            self.no_cars_north.wait_for(self.are_nobody_north)  
            self.ncar_south.value += 1
            self.ncar_waiting_south.value -= 1
            self.car_waiting_south.notify_all()
        if direction == 0 :
            ...
            if self.max_waiting_car_north(): #Condición para que solo se ejecute cuando haya pocos coches del norte esperanod, asi evitamos bloqueo si hay muchos coches en ambos sentidos esperando.
                self.car_waiting_south.wait_for(self.max_waiting_car_south)
            self.no_cars_south.wait_for(self.are_nobody_south) 
            self.ncar_north.value += 1
            self.ncar_waiting_north.value -= 1 
            self.car_waiting_north.notify_all()
        self.ncar_waiting.value -= 1
