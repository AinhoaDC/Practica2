...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 24 12:07:02 2023
...
los otros deben dejarlos pasar. En esta versión comparamos también los coches del norte y sur, viendo la cantidad esperando de cada uno. 
Esta sería la mejor versión de todas realmente, porque tiene en cuenta todos los posibles casos en los que puede ser que 
un proceso no entre. 
"""
import time
import random
...
class Monitor():
    def __init__(self):
        ...
        self.car_waiting_south = Condition(self.mutex)
        self.car_waiting_north = Condition(self.mutex)
        self.ncar = Value ('i', 0)
    ...
    #Usaremos la siguiente función para usarlo de condición como máximo de coches del sur esperando. Con esto tratamos la inanición, evitamos que nunca no entren los coches procedentes del sur. 
    def max_waiting_car_south (self): 
        return self.ncar_waiting_south.value < 7 
    
    #Usaremos la siguiente función para usarlo de condición como máximo de coches del norte esperando. Con esto tratamos la inanición, evitamos que nunca no entren los coches procedentes del norte. 
    def max_waiting_car_north (self): 
        return self.ncar_waiting_north.value < 7
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction == 1:  
            ...
            self.car_waiting_north.wait_for(self.max_waiting_car_north)#Esperamos hasta que se cumpla la condición de que hay menos de 7 coches en el norte esperando. Si se supera esta cifra de coches, los demás tienen que dejarlos pasar. 
            ...
            self.ncar_waiting_south.value -= 1
            self.car_waiting_south.notify_all()#Avisamos a todos cuando no hay coches del sur esperando.
        if direction == 0 :
            self.ncar_waiting_north.value += 1
            if self.max_waiting_car_north(): #Condición para que solo se ejecute cuando haya pocos coches del norte esperanod, asi evitamos bloqueo si hay muchos coches en ambos sentidos esperando.
                self.car_waiting_south.wait_for(self.max_waiting_car_south)#Esperamos hasta que se cumpla la condición de que hay menos de 7 coches en el sur esperando. Si se supera esta cifra de coches, los demás tienen que dejarlos pasar. 
            ...
            self.ncar_waiting_north.value -= 1 
            self.car_waiting_north.notify_all()#Avisamos a todos cuando no hay coches del norte esperando.
        self.ncar_waiting.value -= 1
    ...
    def __repr__(self) -> str:
        return f'Coches: {self.ncar.value}, Peatones: {self.npedestrian_all.value}, Coches_esperando:{self.ncar_waiting.value}, Peatones_esperando:{self.npedestrian_waiting.value}, Coches_esperando_norte:{self.ncar_waiting_north.value},Coches_esperando_sur:{self.ncar_waiting_south.value} '
